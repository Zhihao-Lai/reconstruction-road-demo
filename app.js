...
async function loadCloud(scene, quality = currentQuality) {
  ...
  detailMeta.innerHTML = `
    ...
    <span>全量${formatCount(scene.displayPoints)}点</span>
    <span>当前${cloudSpec.label}${formatCount(cloudSpec.displayPoints)}点</span>
    <span>道路</span>
  `;
...
function renderGrid(items) {
  ...
  for (const item of items) {
    ...
    card.innerHTML = `
      ...
      <div class="case-body">
        <h3>${item.title}</h3>
        <p>全量${formatCount(item.displayPoints)}点 · ${item.inputs.length}张输入图</p>
      </div>
    `;
